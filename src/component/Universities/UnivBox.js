...
const University = (props) => {
 ...
 const [fontColor, setFontColor]= useState("black")
  ...
  function OnUniversityClick(value){
    ...
    if(isSame)
    {dispatch({type:"UNIVERSITIES_DELETE", payload: value})
    setBGColor("transparent")  
    setFontColor("black")
  }
    else
    {dispatch({type: "UNIVERSITIES", payload:value})
    setBGColor(buttonColor)
    setFontColor("white")
}
  }
  
  return(<Uni color={fontColor}background_color={bgcolor} onClick={()=>OnUniversityClick(props.university)}>{props.university}</Uni>)
}
